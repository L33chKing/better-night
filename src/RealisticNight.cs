...
namespace RealisticNight
{
    ...
    public class RealisticNightPlugin : BaseUnityPlugin
    {
        ...
        private void Awake()
        {
            ...
            CityWindowIntensity = Config.Bind("3. City Lights", "City Intensity", 4f, new ConfigDescription("City window emission multiplier. Fades with the universal haze by distance to the nearest lit city street.", new AcceptableValueRange<float>(0f, 100f)));
            ...
            StreetMaxRenderDist = Config.Bind("4. Street Lights", "Max Render Distance (m)", 80000f, new ConfigDescription("Lamps within this distance of the camera render / feed the ground light. Set to your map size (some are 80km+).", new AcceptableValueRange<float>(1000f, 500000f)));
            StreetPoleDrawDist = Config.Bind("4. Street Lights", "Pole Draw Distance (m)", 5000f, new ConfigDescription("LOD: beyond this, only the light ball + ground pool show (no pole mesh). Poles are the heaviest geometry, so this is the main perf lever on huge maps.", new AcceptableValueRange<float>(0f, 500000f)));
            ...
            StreetBuildingLampSearch = Config.Bind("4. Street Lights", "Building Lamp Search (m)", 200f, new ConfigDescription("Second pass: buildings with no streetlamp within this radius get one lamp at their edge, aimed outward (for texture-painted blocks with no real roads). 0 = off.", new AcceptableValueRange<float>(0f, 500f)));

            // --- 5. Atmosphere: the shared air every light shines through ---
            StreetLightAtten = Config.Bind("5. Atmosphere", "Haze Distance (m)", 100000f, new ConfigDescription("UNIVERSAL haze distance for every light the mod adds: street orbs, street pools, headlight pools, cones, decals and city windows all relax toward vanilla past this range (no hard cutoff).", new AcceptableValueRange<float>(100f, 100000f)));
            StreetLightAttenCurve = Config.Bind("5. Atmosphere", "Haze Curve", 0.5f, new ConfigDescription("UNIVERSAL shape of the haze dimming. 1 = natural exponential; higher holds brightness longer then drops fast; lower dims early with a long tail. Live.", new AcceptableValueRange<float>(0.2f, 3f)));
            ...
            StreetSpacingRuralMult = Config.Bind("6. Road Types", "Rural Road Spacing x", 4f, new ConfigDescription("Spacing multiplier for rural roads. 4 = four times the city spacing.", new AcceptableValueRange<float>(1f, 40f)));
            ...
            StreetCityMinRoads = Config.Bind("6. Road Types", "City: min nearby roads", 6, new ConfigDescription("At/above this many nearby roads -> CITY (base spacing, WHITE LED). Raise if too much reads as city.", new AcceptableValueRange<int>(1, 60)));
            StreetMainMinRoads = Config.Bind("6. Road Types", "Main: min nearby roads", 1, new ConfigDescription("At/above this (but below City) -> MAIN road (2x spacing, YELLOW). Below this -> RURAL (4x, YELLOW).", new AcceptableValueRange<int>(0, 30)));
            ...
            StreetWhite = Config.Bind("7. Lamp Orbs", "City Lamp Colour", new Color(0.90f, 0.95f, 1.00f, 1f), "Tint of WHITE (city) lamps - orbs AND pools. Alpha trims the tint strength. Live.");
            StreetYellow = Config.Bind("7. Lamp Orbs", "Sodium Lamp Colour", new Color(1.00f, 0.72f, 0.36f, 1f), "Tint of YELLOW (main + rural) lamps - orbs AND pools. Alpha trims the tint strength. Live.");
            ...
            StreetGroundRange = Config.Bind("8. Ground Pools", "Ground Range (m)", 400f, new ConfigDescription("Radius each lamp lights on the ground. The pool shape normalizes to this range, so raising it also re-brightens already-lit ground.", new AcceptableValueRange<float>(1f, 3000f)));
            ...
            HeadlightIntensity = Config.Bind("9. Headlights", "Cone Brightness", 0.4f, new ConfigDescription("Beam cone + lens HDR brightness (drives bloom).", new AcceptableValueRange<float>(0f, 30f)));
            HeadlightRange = Config.Bind("9. Headlights", "Beam Range (m)", 15f, new ConfigDescription("Length of the volumetric cone. Live — re-posed every frame.", new AcceptableValueRange<float>(5f, 300f)));
            HeadlightConeAngle = Config.Bind("9. Headlights", "Cone Half-Angle (deg)", 28f, new ConfigDescription("Beam spread per lamp. Live — re-posed every frame.", new AcceptableValueRange<float>(3f, 40f)));
            ...
            HeadlightPoolRange = Config.Bind("9. Headlights", "Pool Range (m)", 150f, new ConfigDescription("Ground-pool radius per headlight (own knob, independent of street range). The pool shape normalizes to this range.", new AcceptableValueRange<float>(5f, 2000f)));
            HeadlightPoolFalloff = Config.Bind("9. Headlights", "Pool Falloff", 1f, new ConfigDescription("Pool tightness, NORMALIZED to Pool Range (same look at any range). 0 = flat disc, 1 = tight hot centre. Live.", new AcceptableValueRange<float>(0f, 1f)));
            ...
            HeadlightMaxDist = Config.Bind("9. Headlights", "Max Render Distance (m)", 40000f, new ConfigDescription("Rigs beyond this of the camera hide (cones/decals) and leave the pool feed. (Haze fade itself follows the shared Atmospheric Range.)", new AcceptableValueRange<float>(500f, 100000f)));
            ...
            ShipSearchTargetGate = Config.Bind("10. Ship Lights", "Searchlight Needs Target", true, "Searchlight cone + pool shine only while its turret tracks a live target (a human-operated turret counts as tracking). Off = on all night. Mast-fallback fixtures have no turret and stay on.");
            ShipSearchIntensity = Config.Bind("10. Ship Lights", "Search Pool Intensity", 1f, new ConfigDescription("Searchlight pool brightness.", new AcceptableValueRange<float>(0f, 300f)));
            ...
            ShipSearchPoolFalloff = Config.Bind("10. Ship Lights", "Search Pool Falloff", 1f, new ConfigDescription("Searchlight pool shape (same range-normalized falloff as street pools).", new AcceptableValueRange<float>(0f, 1f)));
            ShipSearchPoolShading = Config.Bind("10. Ship Lights", "Search Pool Shading", 0.9014085f, new ConfigDescription("Searchlight directional shading (same N.L as street pools). 0 = flat naval throw, 1 = angled surfaces read brighter.", new AcceptableValueRange<float>(0f, 1f)));
            ...
            ShipSearchConeColour = Config.Bind("10. Ship Lights", "Search Cone Colour", new Color(0.79f, 1f, 1f, 0.30f), "Beam cone tint. Alpha fades the beam. (Lens dot keeps its warm tint.)");
            ShipSearchSizeScaling = Config.Bind("10. Ship Lights", "Spotlight Size Scaling", 1f, new ConfigDescription("Small turret mounts throw weaker beams (dimmer + shorter) relative to the biggest mount on each hull. 0 = all equal, 1 = fully proportional.", new AcceptableValueRange<float>(0f, 1f)));
